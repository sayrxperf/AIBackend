...
## Function ac.getCustomTonemappingFunctionCode()
...
## Function ac.setFogDensity(value)
  ...
  1. `value`: `number`
## Function ac.setFogAtmosphere(value)

  Parameters:

  1. `value`: `number`
## Function ac.setWeatherParticles(kind, value, size)
Adds extra layer of particles. Currently, requires RainFX module to work. Doesn’t affect anything, purely a visual effect.
  ...
  2. `value`: `number` Intensity from 0 to 1.

  3. `size`: `number?` Particles size adjustment from 0 to 1. Default value: 0.
## Function ac.setSnowMix(value, size)
Replaces falling raindrops with snowflakes. Doesn’t alter anything else at the moment, including windscreen raindrops.

  Parameters:

  1. `value`: `number` Intensity from 0 to 1.

  2. `size`: `number?` Snowflake size adjustment from 0 to 1 (leave at -1 for an automatic value). Default value: -1.
## Function ac.setRainWindscreenDropsMultiplier(value)
Replaces falling raindrops with snowflakes. Doesn’t alter anything else at the moment, including windscreen raindrops.

  Parameters:

  1. `value`: `number` Intensity from 0 to 1.
## Function ac.configureGrassShading(specularity, reflectivity, backlit, saturation)
Adjusts GrassFX shading to avoid cartoonish flat look.

  Parameters:

  1. `specularity`: `number`

  2. `reflectivity`: `number`

  3. `backlit`: `number?` Added in 0.2.8, default value is 0. Default value: 0.

  4. `saturation`: `number?` Added in 0.2.8, default value is 1. Default value: 1.
## Function ac.setNearbyFog(color, distance, heightFactor, transparency, exp)
  ...
  3. `heightFactor`: `number`

  4. `transparency`: `number?` Default value: 1.

  5. `exp`: `number?` Default value: 1.
...
## Function ac.setCloudShadowIndependantOpacity(value)
  ...
  1. `value`: `boolean|`true`|`false``
## Function ac.setCloudShadowBlur(amount, fadeWithDistance)
Added in 0.3.0.

  Parameters:

  1. `amount`: `number` 0 to disable blur (a tiny amount would still be applied to prevent flicker, 1 for blurring of 0.2 builds.

  2. `fadeWithDistance`: `number?` -1 to disable fade out, 0 to disable to shadows where there is no data (recommended), 1 for fully shadowed distant clouds. Default value: -1.
...
## Function ac.setCloudArcMultiplier(value)
  ...
  1. `value`: `number`
## Function ac.disableSkyRender(disable)
...
## Function ac.setDistantAmbientColor(c, distance)
  ...
  2. `distance`: `number`
## Function ac.setRadiosityAmbientColor(c, exp)
...
## Function ac.setExtraAmbientDirection(c)
  ...
  1. `c`: `vec3`
## Function ac.setExtraAmbientDistance(d)
...
## Function ac.setSkyMoonTexture(v, maxSize)
  ...
  2. `maxSize`: `integer?` Default value: 0.
## Function ac.setEarthTexture(v, maxSize)
...
## Function ac.getSkyMoonTextureState()
  ...
  - `ac.TextureState`
## Function ac.getEarthTextureState()
...
## Function ac.setSkyMoonGradient(v)
  ...
  1. `v`: `number`
## Function ac.setSkyMoonClipThreshold(v)
...
## Function ac.setSkyV2Gamma(side, v)
  ...
  2. `v`: `number`
## Function ac.setReflectedSkyTweaks(luminance, saturation, gamma)
Doesn’t affect sky reflected from screen-space data. Ideally, try not to use it.

  Parameters:

  1. `luminance`: `number?` Default value: 1.

  2. `saturation`: `number?` Default value: 1.

  3. `gamma`: `number?` Default value: 1.
...
## Function ac.setPpColorGradingIntensity(v)
  ...
  1. `v`: `number`
## Function ac.pauseCubemapUpdates(pause)

  Parameters:

  1. `pause`: `boolean?` Default value: `true`.
## Function ac.setHDRToLDRConversionHints(brightnessMult, exponent)
Used for cases where cheap HDR-to-LDR conversion is needed, for example, in extra car cameras.

  Parameters:

  1. `brightnessMult`: `number` Applies to HDR image before exponent.

  2. `exponent`: `number`
## Function ac.useLinearColorSpace(active, lightsDivisor)
...
fog layer as well, also changes behaviour of some WeatherFX style functions fixing multiple bugs.
  ...
  2. `lightsDivisor`: `number?` Used to scale lights to try and prevent clipping due to limited float16 precision. Default value: 1.
...
## Function ac.setWeatherLightsMultiplier(value)
  ...
  1. `value`: `number`
## Function ac.setWeatherLightsRangeFactor(value)
...
## Function ac.setWeatherTrackLightsMultiplierThreshold(value)
  ...
  1. `value`: `number`
## Function ac.setWeatherLightsMultiplier2(value)
...
## Function ac.setAutoExposureLimits(min, max)
  ...
  2. `max`: `number`
## Function ac.useMinDepthResolution(v)
...
## Function ac.fixSkyV2Fog(value)
  ...
  1. `value`: `boolean|`true`|`false``
## Function ac.setSkyV2DitherScale(scale)
...
## Function ac.testFrustumIntersection(v, radius, applyFixHeading)
  ...
  - `boolean`
## Function ac.calculateSkyColor(v, includeSun, includeMoonColor, includeSunMie)

  Parameters:

  1. `v`: `vec3`

  2. `includeSun`: `boolean|`true`|`false``

  3. `includeMoonColor`: `boolean|`true`|`false``

  4. `includeSunMie`: `boolean?` Default value: `true`.

  Returns:

  - `rgb`
## Function ac.calculateSkyColorTo(r, v, includeSun, includeMoonColor, includeSunMie)
  ...
  3. `includeSun`: `boolean|`true`|`false``

  4. `includeMoonColor`: `boolean|`true`|`false``

  5. `includeSunMie`: `boolean?` Default value: `true`.
## Function ac.calculateSkyColorV2(v, includeSun, includeMoonColor, includeSunMie)
  ...
  - `rgb`
## Function ac.calculateSkyColorV2To(r, v, includeSun, includeMoonColor, includeSunMie)
  ...
  5. `includeSunMie`: `boolean?` Default value: `true`.
## Function ac.calculateSkyColorNoGradients(v, includeSun, includeMoonColor, includeSunMie)
  ...
  - `rgb`
## Function ac.calculateSkyColorNoGradientsTo(r, v, includeSun, includeMoonColor, includeSunMie)
...
## Function ac.setManualCloudsInvalidation(value)
  ...
  1. `value`: `boolean|`true`|`false``
## Function ac.adjustCubemapReprojectionBrightness(value)

  Parameters:

  1. `value`: `number`
## Function ac.enableRenderCallback(sky, clouds, track, cloudShadows, sceneEnd)
Note: `track`, `cloudShadows` and `sceneEnd` callbacks are only called with main pass ID.

  Parameters:

  1. `sky`: `render.PassID?` Default value: `render.PassID.None`.

  2. `clouds`: `render.PassID?` Default value: `render.PassID.None`.

  3. `track`: `render.PassID?` Default value: `render.PassID.None`.

  4. `cloudShadows`: `render.PassID?` Default value: `render.PassID.None`.

  5. `sceneEnd`: `render.PassID?` Default value: `render.PassID.None`.
## Function ac.getPostProcessingHDRColorMatrix()
To be used from `ac.onPostProcessing()` callback if you want to sync with color adjustments.

  Returns:

  - `mat4x4`
## Function ac.getPostProcessingLDRColorMatrix()
...
LDR color matrix will be applied automatically to the final output.
  ...
  - `mat4x4`
## Function ac.getPostProcessingColorGradingIntensity()
To be used from `ac.onPostProcessing()` callback if you want to sync with color grading. Use `dynamic::pp::colorGrading3D` as texture name to use it properly.
  ...
  - `number` Returns 0 if color grading is disabled or failed to load.
## Function ac.setAiHeadlightsSuggestion(v, vaoThreshold)
...
if automatically switching headlights are active (threshold for player is always 0.5).
 ...
 Lights can’t change more than once per second.
  ...
  2. `vaoThreshold`: `number?` Lights would also activate if car VAO is below this value. Default value: 0.
...
`ac.getCubemapBrightnessEstimationMaximum()`, `ac.getCubemapBrightnessEstimationMinimum()` and `ac.getCubemapBrightnessEstimationAverage()`.
  ...
  1. `zoom`: `number?` Set above 0 and below 1 to zoom in (1 zooms in a single point in front of camera), set below 0 to expand region taken into account. You might need to get camera FOV with `ac.getSim().cameraFOV` to calculate optimal zoom value for your case. Default value: 0.
...
- Update happens once every two frames, and has a delay for about ten frames (which is inevitable with how GPUs work, delay so high is to make sure it would never cause a lock and a corresponding FPS drop)
  ...
  - `number` Returns average brightness, or -1 if it’s not yet available (hasn’t been computed yet, or if ReflectionsFX is disabled).
## Function render.tornado(pos, scale, alpha, time)
...
Sets value of a track condition input.
  ...
  1. `key`: `string` Name of an input.

  2. `value`: `number` New input value.
## Function ac.overrideVirtualMirrorShader(v)
...
```
  ...
  1. `v`: `{values: table, defines: table, shader: string, cacheKey: integer}?` Pass `nil` to disable override.
## Class ScriptData
...
- `ac.SkyCloudBase:setTexture(filename)`
...
- `ac.SkyCloudBase:getTextureState()`
...
- `ac.SkyCloud:setNoiseTexture(filename)`
...
- `ac.SkyCloud:getNoiseTextureState()`
...
texture is quite heavy and takes some time to generate, so don’t change those settings live.
  ...
  - `ac.SkyCloudMapParams`
## Class ac.SkyCloudMapParams
Clouds v2 use a special 3D noise texture to change shape live. This structure describes parameters for generating said texture. Note:
texture is quite heavy and takes some time to generate, so don’t change those settings live.
## Function ac.SkyCloudMapParams.new()

  Returns:

  - `ac.SkyCloudMapParams`

# Module wfx_impl/ac_cloudscovers.lua

## Function ac.SkyCloudsCover()
...
- `ac.SkyCloudsCover:setTexture(filename, maxSize)`

  Parameters:

    1. `filename`: `string`

    2. `maxSize`: `number?` If non-zero, sets a maximum size for MIPs to load. Any MIPs larger than that will be skipped. Only works with BC6H/BC7 compression, or with DXT1/3/5/RGBA8888 if using new loader option is enabled.

- `ac.SkyCloudsCover:getTextureState()`

  function _ac_SkyCloudsCover:getTextureState() end

- `ac.SkyCloudsCover:setMaskTexture(filename, maxSize)`
...
- `ac.SkyCloudsCover:getMaskTextureState()`

  function _ac_SkyCloudsCover:getMaskTextureState() end

- `ac.SkyCloudsCover:setFogParams(fogHorizon, fogZenith, fogExponent, fogRangeMult)`

  function _ac_SkyCloudsCover:setFogParams(fogHorizon, fogZenith, fogExponent, fogRangeMult) end

# Module wfx_impl/ac_gradients.lua

## Function ac.SkyExtraGradient(t)

  Parameters:

  1. `t`: `{` direction: vec3, sizeFull: number, sizeStart: number, isAdditive: boolean, isIncludedInCalculate: boolean, color: rgb, exponent: number }? 
...
# Module wfx_impl/ac_particlematerials.lua

## Function ac.ParticlesMaterial(emissiveBlend, diffuse, ambient)

  Parameters:

  1. `emissiveBlend`: `number`
  ...
  - `ac.ParticlesMaterial`
## Class ac.ParticlesMaterial

# Module wfx_impl/ac_lightpollution.lua

## Function ac.LightPollution(position, radius, tint, density)
...
# Module wfx_impl/ac_obsolete.lua

## Function ac.setSkySunSize(v)
## Function ac.setSunAngle(v)
## Function ac.setCustomSunDirection(v)
## Function ac.setReflectionsLuminanceBoost(v)
## Function ac.setSkyExtraGradient(id, gradient)

# Module wfx_impl/ac_lists.lua

## Class ac.WeatherCloudsList
List of active ac.SkyCloudBase. Use it to add or remove elements to the scene.

- `ac.WeatherCloudsList:get(index)`
...
- `ac.WeatherCloudsList:set(index, item)`
  ...
  Parameters:
    ...
    2. `item`: `ac.SkyCloudBase`

- `ac.WeatherCloudsList:insert(index, item)`
...
- `ac.WeatherCloudsList:push(item)`

  Parameters:

    1. `item`: `ac.SkyCloudBase`

- `ac.WeatherCloudsList:pushWhereFits(item)`
...
- `ac.WeatherCloudsList:erase(item)`
...
- `ac.WeatherCloudsList:remove(index)`
  ...
  Returns:
    ...
    - `ac.SkyCloudBase` Removed element.

- `ac.WeatherCloudsList:pop()`
  ...
  Returns:
    ...
    - `ac.SkyCloudBase`
## Function ac.weatherClouds:get(index)
  ...
  - `ac.SkyCloudBase`
## Function ac.weatherClouds:set(index, item)
If item is null, element will be removed from said position, moving rest one step forward to close the gap.
  ...
  2. `item`: `ac.SkyCloudBase`
## Function ac.weatherClouds:insert(index, item)
  ...
  2. `item`: `ac.SkyCloudBase`
## Function ac.weatherClouds:push(item)

  Parameters:

  1. `item`: `ac.SkyCloudBase`
## Function ac.weatherClouds:pushWhereFits(item)
Inserts element to a first empty spot.

  Parameters:

  1. `item`: `ac.SkyCloudBase`
## Function ac.weatherClouds:erase(item)
Removes an element from the list (first occurance only).

  Parameters:

  1. `item`: `ac.SkyCloudBase`
## Function ac.weatherClouds:remove(index)
Removes an element from a position, moves the rest one step forward to close the gap.
  ...
  - `ac.SkyCloudBase` Removed element.
## Function ac.weatherClouds:pop()
Removes the last element and returns it.

  Returns:

  - `ac.SkyCloudBase`
## Class ac.WeatherCloudsCoversList
List of active ac.SkyCloudsCover. Use it to add or remove elements to the scene.

- `ac.WeatherCloudsCoversList:get(index)`
...
- `ac.WeatherCloudsCoversList:set(index, item)`
  ...
  Parameters:
    ...
    2. `item`: `ac.SkyCloudsCover`

- `ac.WeatherCloudsCoversList:insert(index, item)`
...
- `ac.WeatherCloudsCoversList:push(item)`

  Parameters:

    1. `item`: `ac.SkyCloudsCover`

- `ac.WeatherCloudsCoversList:pushWhereFits(item)`
...
- `ac.WeatherCloudsCoversList:erase(item)`
...
- `ac.WeatherCloudsCoversList:remove(index)`
  ...
  Returns:
    ...
    - `ac.SkyCloudsCover` Removed element.

- `ac.WeatherCloudsCoversList:pop()`
  ...
  Returns:
    ...
    - `ac.SkyCloudsCover`
## Function ac.weatherCloudsCovers:get(index)
  ...
  - `ac.SkyCloudsCover`
## Function ac.weatherCloudsCovers:set(index, item)
If item is null, element will be removed from said position, moving rest one step forward to close the gap.
  ...
  2. `item`: `ac.SkyCloudsCover`
## Function ac.weatherCloudsCovers:insert(index, item)
  ...
  2. `item`: `ac.SkyCloudsCover`
## Function ac.weatherCloudsCovers:push(item)

  Parameters:

  1. `item`: `ac.SkyCloudsCover`
## Function ac.weatherCloudsCovers:pushWhereFits(item)
Inserts element to a first empty spot.

  Parameters:

  1. `item`: `ac.SkyCloudsCover`
## Function ac.weatherCloudsCovers:erase(item)
Removes an element from the list (first occurance only).

  Parameters:

  1. `item`: `ac.SkyCloudsCover`
## Function ac.weatherCloudsCovers:remove(index)
Removes an element from a position, moves the rest one step forward to close the gap.
  ...
  - `ac.SkyCloudsCover` Removed element.
## Function ac.weatherCloudsCovers:pop()
Removes the last element and returns it.

  Returns:

  - `ac.SkyCloudsCover`
## Class ac.SkyExtraGradientsList
List of active ac.SkyExtraGradient. Use it to add or remove elements to the scene.

- `ac.SkyExtraGradientsList:get(index)`
...
- `ac.SkyExtraGradientsList:set(index, item)`
  ...
  Parameters:
    ...
    2. `item`: `ac.SkyExtraGradient`

- `ac.SkyExtraGradientsList:insert(index, item)`
...
- `ac.SkyExtraGradientsList:push(item)`

  Parameters:

    1. `item`: `ac.SkyExtraGradient`

- `ac.SkyExtraGradientsList:pushWhereFits(item)`
...
- `ac.SkyExtraGradientsList:erase(item)`
...
- `ac.SkyExtraGradientsList:remove(index)`
  ...
  Returns:
    ...
    - `ac.SkyExtraGradient` Removed element.

- `ac.SkyExtraGradientsList:pop()`
  ...
  Returns:
    ...
    - `ac.SkyExtraGradient`
## Function ac.skyExtraGradients:get(index)
  ...
  - `ac.SkyExtraGradient`
## Function ac.skyExtraGradients:set(index, item)
If item is null, element will be removed from said position, moving rest one step forward to close the gap.
  ...
  2. `item`: `ac.SkyExtraGradient`
## Function ac.skyExtraGradients:insert(index, item)
  ...
  2. `item`: `ac.SkyExtraGradient`
## Function ac.skyExtraGradients:push(item)

  Parameters:

  1. `item`: `ac.SkyExtraGradient`
## Function ac.skyExtraGradients:pushWhereFits(item)
Inserts element to a first empty spot.

  Parameters:

  1. `item`: `ac.SkyExtraGradient`
## Function ac.skyExtraGradients:erase(item)
Removes an element from the list (first occurance only).

  Parameters:

  1. `item`: `ac.SkyExtraGradient`
## Function ac.skyExtraGradients:remove(index)
Removes an element from a position, moves the rest one step forward to close the gap.
  ...
  - `ac.SkyExtraGradient` Removed element.
## Function ac.skyExtraGradients:pop()
Removes the last element and returns it.

  Returns:

  - `ac.SkyExtraGradient`
## Class ac.WeatherColorCorrectionsList
List of active ac.ColorCorrectionBase. Use it to add or remove elements to the scene.

- `ac.WeatherColorCorrectionsList:get(index)`
  ...
  Returns:
    ...
    - `ac.ColorCorrectionBase`

- `ac.WeatherColorCorrectionsList:set(index, item)`
  ...
  Parameters:
    ...
    2. `item`: `ac.ColorCorrectionBase`

- `ac.WeatherColorCorrectionsList:insert(index, item)`
...
- `ac.WeatherColorCorrectionsList:push(item)`

  Parameters:

    1. `item`: `ac.ColorCorrectionBase`

- `ac.WeatherColorCorrectionsList:pushWhereFits(item)`
...
- `ac.WeatherColorCorrectionsList:erase(item)`
...
- `ac.WeatherColorCorrectionsList:remove(index)`
  ...
  Returns:
    ...
    - `ac.ColorCorrectionBase` Removed element.

- `ac.WeatherColorCorrectionsList:pop()`
  ...
  Returns:
    ...
    - `ac.ColorCorrectionBase`
## Function ac.weatherColorCorrections:get(index)
  ...
  - `ac.ColorCorrectionBase`
## Function ac.weatherColorCorrections:set(index, item)
If item is null, element will be removed from said position, moving rest one step forward to close the gap.
  ...
  2. `item`: `ac.ColorCorrectionBase`
## Function ac.weatherColorCorrections:insert(index, item)
  ...
  2. `item`: `ac.ColorCorrectionBase`
## Function ac.weatherColorCorrections:push(item)

  Parameters:

  1. `item`: `ac.ColorCorrectionBase`
## Function ac.weatherColorCorrections:pushWhereFits(item)
Inserts element to a first empty spot.

  Parameters:

  1. `item`: `ac.ColorCorrectionBase`
## Function ac.weatherColorCorrections:erase(item)
Removes an element from the list (first occurance only).

  Parameters:

  1. `item`: `ac.ColorCorrectionBase`
## Function ac.weatherColorCorrections:remove(index)
Removes an element from a position, moves the rest one step forward to close the gap.
  ...
  - `ac.ColorCorrectionBase` Removed element.
## Function ac.weatherColorCorrections:pop()
Removes the last element and returns it.
...
# Module wfx_impl/ac_customtonemapping.lua
...
Override current tonemapping function. If you’re using a table and need texture coordinates, add `__CSP_PROVIDE_TEXCOORDS = true` define.
  ...
  1. `v`: `ac.TonemapFunction|string|{textures: table, values: table, defines: table, shader: string, cacheKey: integer}`

# Module wfx_impl/ac_postprocessing.lua
