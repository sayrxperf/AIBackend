...
or offset its image (if you want some transition animation, for example). Set opacity to 0 and navigator would stop.
  ...
  3. `offset`: `number|vec2?` Offset for an output of a given screen entity. Default value: `vec2(0, 0)`.

  4. `scale`: `number|vec2?` Scale for an output of a given screen entity. Default value: `vec2(1, 1)`.
...
free. Also, raycasting only happens if mouse is clicked. And, of course, found meshes are cached as well.
  ...
  2. `uvFrom`: `number|vec2?` UV region, start (from 0 to 1). Default value: `vec2(0, 0)`.

  3. `uvSize`: `number|vec2?` UV region, end (from 0 to 1). Default value: `vec2(1, 1)`.
...
## Function ac.setNavigatorText(key, value)
...
## Function ac.accessCarLight(name)
...
Note: smaller rays are faster to cast. Also, rays going straight down are slightly faster as well.
  ...
  - `number`
## Function ac.accessCarRenderingCamera(name)
Access rendering camera data (for both reading and writing). For faster access cache the result.

  Parameters:

  1. `name`: `string` Name of a car section.

  Returns:

  - `ac.StateCarRenderingCamera`
## Function ac.accessCarTexturePreprocessing(name)
Access rendering camera data (for both reading and writing). For faster access cache the result.
  ...
  - `ac.StateCarTexturePreprocessing`
...
# Module car_scriptable_display/car_scriptable_display_structs.lua
...
- `display.DrawnMirror:project(pos)`
...
verify if point is inside or outside.
  ...
  Parameters:
    ...
    1. `pos`: `vec3` Point in world coordinates.
