...
whole AC would pause waiting for it.
  ...
  Parameters:
    ...
    1. `dt`: `number` Same time as was just passed to `update()`.

- `ScriptData.simUpdate(dt)`
...
objects and such.
  ...
  Parameters:
    ...
    1. `dt`: `number` How much time has passed since last call, in seconds.

- `ScriptData.frameBegin(dt, gameDT)`
  ...
  Parameters:
    ...
    2. `gameDT`: `number` How much time has passed in simulation (slower with slow-mo replays, 0 when paused), in seconds.
