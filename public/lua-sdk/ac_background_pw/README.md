# Library ac_background_pw

Documentation for ac_background_pw. Please note: documentation generator is in development and needs more work, so some information might be missing and other bits might not be fully accurate.
...
## Function web.encryptKey(key)
  ...
  - `string`
## Function physics.getBodyTransform(carIndex, wheelIndex, inverse)
Returns a transformation matrix of a rigidbody.
  ...
  2. `wheelIndex`: `integer?` 0-based index, or -1 for body transform. Default value: -1.

  3. `inverse`: `boolean?` Pass `true` to get an inverted matrix. Default value: `false`.

  Returns:

  - `mat4x4`
## Function physics.getWheelAngularSpeed(carIndex, wheelIndex)
Returns angular speed of a wheel.
  ...
  2. `wheelIndex`: `integer` 0-based index.
