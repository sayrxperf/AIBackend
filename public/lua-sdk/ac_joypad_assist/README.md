...
## Function ac.getJoypadState()

  Returns:

  - `ac.StateJoypadData`
## Function ac.setDualSenseTriggerNoEffect(triggerIndex)

  Parameters:

  1. `triggerIndex`: `integer` 0 for left, 1 for right.
## Function ac.setDualSenseTriggerContinuousResitanceEffect(triggerIndex, startPosition, force)

  Parameters:

  1. `triggerIndex`: `integer` 0 for left, 1 for right.

  2. `startPosition`: `number` Start position of resistance.

  3. `force`: `number` Force of resistance.
## Function ac.setDualSenseTriggerSectionResitanceEffect(triggerIndex, startPosition, endPosition)
  ...
  3. `endPosition`: `number` End position of resistance (>= start).
## Function ac.setDualSenseTriggerExtendedEffect(triggerIndex, startPosition, beginForce, middleForce, endForce, frequency, keepEffect)

  Parameters:

  1. `triggerIndex`: `integer` 0 for left, 1 for right.

  2. `startPosition`: `number` Position at which the effect starts.

  3. `beginForce`: `number` Force applied when trigger <= 0.5.

  4. `middleForce`: `number` Force applied when trigger >= 0.5.

  5. `endForce`: `number` Force applied when trigger is beyond 1.

  6. `frequency`: `number` Vibration frequency of the trigger (more like a 0�1 value to scale between two fixed frequencies).

  7. `keepEffect`: `boolean|`true`|`false`` Set to `true` to keep the effect playing when trigger goes beyond 1.
## Function ac.setDualSenseHapticParams(gamepadIndex, key, value)
Overrides haptic vibration parameter for the given key.

  Parameters:

  1. `gamepadIndex`: `integer`

  2. `key`: `ac.DualSenseHapticParam`

  3. `value`: `vec4`
## Function ac.setCustomDriverModelSteerAngle(angle)
